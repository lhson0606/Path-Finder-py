...
def get_neighbors(current, m: Map):
    neighbors = []
    ...
    for i in range(-1, 2):
        for j in range(-1, 2):
            for k in range(-1, 2):
                ...
                if m.is_wall(glm.ivec3(current.x + i, current.y + j, current.z + k)):
                    continue
...
def a_star_search(start: glm.ivec3, goal: glm.ivec3, m: Map):
    open_set = [(0, start)]
    came_from = {}
    ...
    while len(open_set) > 0:
        ...
        open_set.pop(0)
        for neighbor in get_neighbors(current, m):
...
class VisualizedAStar(VisualizedAlgorithm.VisualizedAlgorithm):
    def find_path(self):
        return a_star_search(self.map.map.start, self.map.map.goal, self.map.map)
        pass
